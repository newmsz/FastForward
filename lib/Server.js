...
Server.prototype.listen = function () {
	var listening_port = this._listen_port;
	Logger.info('Listening ' + listening_port + '...');
	...
	this._server.on('error', function (err) {
		if(err && err.code == 'EADDRINUSE') {
			console.error('FATAL: ADDRESS IS IN USE: ' + listening_port);
			process.exit(1);
		} else {
			console.error(err);
		}
...
Server.prototype.setTimeout = function (ms) {
	for(var i=0; i<this._locations.length; i++)
		this._locations[i].setTimeout(ms);
	return this;
};
