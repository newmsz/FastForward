...
Location.prototype.tryForward = function (src_req, src_res, logger) {
	if(this._forward_to) {
		...
		var dst_req = this._forward_to.upstream.openTransport(this._forward_to.transport, options, function (dst_res) {
			...
			dst_res.on('error', function (err) { console.error('Destination response error: '); console.error(err); });
			src_req.on('error', function (err) { console.error('Source request error: '); console.error(err); });
			src_res.on('error', function (err) { console.error('Source response error: '); console.error(err); });
