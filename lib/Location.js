...
Location.prototype.tryForward = function (src_req, src_res, logger) {
	if(this._forward_to) {
		...
		var dst_req = this._forward_to.upstream.openTransport(this._forward_to.transport, options, function (dst_res) {
			...
			dst_res.on('data', function (chunk) { src_res.write(chunk); log_primitives.bytes_sent += chunk.length; });
			dst_res.on('end', function () { src_res.end(); logger && logger.log(log_primitives); });
		});
