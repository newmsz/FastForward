...
Location.prototype.tryRewrite = function (src_req, src_res) {
	if(this._rewrite) {
		...
		to = to.replace('$query', incoming_url.query || '');
...
Location.prototype.tryForward = function (src_req, src_res, logger) {
	if(this._forward_to) {
		...
		var dst_req = this._forward_to.upstream.openTransport(this._forward_to.transport, options, function (dst_res) {
			...
			src_req.on('error', function (err) { console.error('Source request error: ' + err); });
			src_res.on('error', function (err) { console.error('Source response error: ' + err); });
			dst_req.on('error', function (err) { console.error('Destination request error: ' + err); });
			dst_res.on('error', function (err) { console.error('Destination response error: ' + err); });
